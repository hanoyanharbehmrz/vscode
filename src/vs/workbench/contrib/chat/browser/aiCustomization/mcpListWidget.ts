...
export class McpListWidget extends Disposable {
	...
	private updateAccessState(): void {
		...
		if (disabled) {
			...
			if (policyLocked) {
				this.disabledMessage.textContent = localize('mcpAccessDisabledByPolicy', "Access to MCP servers is disabled by your organization. Contact your administrator if you need to install or run MCP servers in chat.");
			} else {
